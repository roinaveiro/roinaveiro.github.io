...
subtitle: "SAMSI"
author: "Roi Naveiro and David Ríos"
date: "roi.naveiro@icmat.es | Inst. of Mathematical Sciences (ICMAT-CSIC)"
output:
...
* Our interest is in Stackelberg Games arising in AML.

* In classical games, decision spaces are small. In AML, $\alpha$ and $\beta$ usually **high dimensional** and **continuous** (e.g. weights of a NN).
---
...
* $R_J$, aware of the actual superiority of its competitor's wines, decides to hack $R_D$'s system by manipulating the value of several quality indicators, to artificially decrease $R_D$'s quality rates. 
<center>
...
# Application to AML - Adversarial Regression
* $R_D$ is aware of the possibility of being hacked and decides to train its regression in an **adversarial robust** manner.

*  $R_D$ models the situation as a game between a *learner* and a *data generator*.

* The *data generator* tries to fool the learner modifying input data at application time, inducing
a change between the data distribution at training and test times.
...
* The data generator aims at choosing the transformation that minimizes the theoretical costs given by
...
* Two solution methods: forward and backward.

* Forward scales well in space, backward scales well in time.

* We are able to solve **huge Stackelberg Games** (thousands of decisions).
...
* Games with uncertainty, i.e. $\Psi_D(\alpha, \beta) = \int u_D(\alpha, \theta) p(\theta \vert \alpha, \beta) \mathop{}\! \mathrm{d}  \theta$ and $\Psi_A(\alpha, \beta) = \int u_A(\beta, \theta) p(\theta \vert \alpha, \beta) \mathop{}\! \mathrm{d}  \theta$.

* Bayesian Stackelberg Games? I model my uncertainty about Attacker's utility using $u_A(\alpha, \beta, \tau)$ and optimize $\int u_A(\alpha, \beta, \tau) p(\tau) \mathop{}\! \mathrm{d}  \tau$.
