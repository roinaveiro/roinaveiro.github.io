...
* **Common Knowledge Assumtion**: the Defender nows the Attacker's probabilities and utilities.
...
* Attacker's best respons to defense $d$
\begin{equation}
...
* Efficient if uncertainty does not depend on decision.
---
# APS solution method
...
* This approach requires generating **at most** $N \times (2 \times M)+ 2 \times N$.

* We get rid off the cardinality of decision spaces.
...
* Allows simulating from a **power transformation**, as in ().
...
* Attacker's payoff: $g(d, \theta) = (1-\theta) \times s - c \times d$.
...
* We are able to get rid off the cardinality of the attacker's decision space, but not that of the Defender's.
